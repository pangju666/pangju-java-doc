...
### 概述
我默认装配了一个`RestClient`并向其配置了[响应内容缓存拦截器](/v1/framework/web/client#响应内容缓存拦截器)来配合[`错误处理器`](/v1/framework/web/client#错误处理器)
...
## 异常信息过滤器
我默认装配了一个[异常列表过滤器](/v1/framework/web/filter#http异常信息过滤器)
