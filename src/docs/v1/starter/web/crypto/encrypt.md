...
支持对`String`、`byte[]`和`JSON`类型的响应体进行加密，加密失败则抛出[`ServerException`](/v1/framework/web/exception#服务器内部异常)。
...
>
> `#headers.getFirst('My-Secret-Header')` - 使用请求头参数
>
...
- key: 明文密钥或占位符，支持三种形式：
  ...
  3. SpEL表达式：例如：`#headers.getFirst('My-Secret-Header')`，使用`headers`作为上下文变量
