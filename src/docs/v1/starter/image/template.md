...
### 操作配置
[`io.github.pangju666.framework.boot.image.model.GenericImageOperation`](/v1/starter/image/model#通用)
...
- 使用`Tika`进行图像类型解析。
- 使用`ImageMetadataReader`和[`ImageUtils`](/v1/commons/image/utils)读取图像尺寸与`EXIF`方向，并封装为[`ImageFile`](/v1/starter/image/model#图像信息)。
- 使用[`ImageEditor`](/v1/commons/image/editor)进行图像处理。

### 格式支持
- 读取类型以[`ImageIO`支持读取的类型](/v1/commons/image/constants)判定。
- 写入格式以[`ImageIO`支持写入的类型](/v1/commons/image/constants)判定。

### 操作配置
[`io.github.pangju666.framework.boot.image.model.BufferedImageOperation`](/v1/starter/image/model#bufferedimage)
...
### 异常与容错
- 元数据解析失败时使用[默认方向](/v1/commons/image/constants)并通过文件解码获取尺寸。
- 输入或输出类型不受支持时抛出[不支持的图片类型异常](/v1/starter/image/exception#不支持的图片类型异常)。
- 图像处理过程中发生错误时抛出[图像操作异常](/v1/starter/image/exception#图像操作异常)
...
> `gif`格式不支持裁剪。
> 
> `gif`格式如果某些帧尺寸小于指定的锐化半径，会导致抛出[`ImageOperationException`](/v1/starter/image/exception#图像操作异常)异常，但是图片会正常生成。
...
### 格式支持
- 读取格式以[`GraphicsMagick`支持读取的类型](/v1/starter/image/constants)判定。
- 写入格式以[`GraphicsMagick`支持写入的类型](/v1/starter/image/constants)判定。

### 操作配置
[`io.github.pangju666.framework.boot.image.model.GMImageOperation`](/v1/starter/image/model#graphicsmagick)
...
### 异常与容错
- 输入或输出类型不受支持时抛出[不支持的图片类型异常](/v1/starter/image/exception#不支持的图片类型异常)。
- `GraphicsMagick`命令失败或进程通信错误时抛出[图像操作异常](/v1/starter/image/exception#图像操作异常)
