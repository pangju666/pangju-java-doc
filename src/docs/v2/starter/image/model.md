...
> [!NOTE]
> 图像文件摘要计算基于[`xxHash64`](/v2/commons/io/file#计算文件摘要)算法。
> 
...
- 一般从[`ImageTemplate`](/v2/starter/image/template#接口)获取。

### 字段
- imageSize：图像宽高信息，见[`ImageSize`](/v2/commons/image/model#图像尺寸)。
- orientation：图像方向标记（`EXIF Orientation`），默认值见[`ImageConstants.NORMAL_EXIF_ORIENTATION`](/v2/commons/image/constants)。
...
#### 概述
通用图像处理操作配置模型，像[`ImageTemplate`](/v2/starter/image/template#接口)提供操作配置。
...
- 水印 (Watermark)：
  - 类型：支持图片水印与文字水印（互斥）。
  - 定位：支持[九宫格方位](/v2/commons/image/enums#水印方向)与绝对坐标定位（互斥）。
...
#### 使用示例
以下的操作设置属于通用操作，所有[`ImageTemplate`](/v2/starter/image/template#接口)实现都应支持。
...
#### 概述
面向[`BufferedImageTemplate`](/v2/starter/image/template#bufferedimage实现)的图像操作配置。

#### 拓展操作说明
- 缩放优化：
  - 支持指定[重采样过滤器](/v2/starter/image/enums#重采样过滤器)以控制缩放质量。
- 图像增强：
  - 模糊：高斯模糊（支持自定义半径）；
...
#### 概述
面向[`GMImageTemplate`](/v2/starter/image/template#gm实现)的图像操作配置。
...
- 缩放优化：支持指定[重采样过滤器](/v2/starter/image/enums#重采样过滤器)以控制缩放质量。
- 图像增强：
