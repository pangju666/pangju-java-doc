...
我提供了一个JSON响应错误处理器，当请求失败的时候，会自动处理并抛出一个[HTTP远程服务异常](/v2/framework/web/exception#http远程服务异常)，
如果响应状态码是`504`，则会抛出[HTTP远程服务异常](/v2/framework/web/exception#http远程服务超时异常)。
...
3. `RestClient`必须注册[响应体缓冲拦截器](/v2/framework/web/client#响应内容缓存拦截器)，不然的话响应状态码为`200`的业务性错误是直接忽略掉的。
:::
...
主要是用来定义错误信息的，用途可以参考[远程服务异常](/v2/framework/web/exception#远程服务异常)。
